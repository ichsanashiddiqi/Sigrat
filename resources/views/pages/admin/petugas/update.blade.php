                            ...
                            <div class="card-body">
                                <div class="tab-content">
                                    <div class="active tab-pane" id="list">
                                        <form class="form-horizontal" action="{{ route('petugas.update', $data->id) }}"
                                            method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')

                                            <div class="form-group row">
                                                <label for="inputName" class="col-sm-2 col-form-label">Nama Surat Tugas</label>
                                                <div class="col-sm-10">
                                                    <select class="form-control" name="surat_tugas" id="employees">
                                                        @foreach ($outbox as $outbox)
                                                            <option value="{{ $outbox->id }}" {{$data->surat_tugas_id === $outbox->id ? 'selected' : ''}}>
                                                                {{ $outbox->nama_surat }}
                                                            </option>
                                                        @endforeach
                                                    </select>
